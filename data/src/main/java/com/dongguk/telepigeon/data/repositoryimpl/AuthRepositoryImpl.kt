...
class AuthRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun postLogin(authorization: String): Result<AuthTokenModel> =
            runCatching {
                authRemoteDataSource.postLogin(authorization = authorization).data.toAuthTokenModel()
            }

        override suspend fun deleteLogout(): Result<Unit> =
            runCatching {
                authRemoteDataSource.deleteLogout()
            }

        override suspend fun deleteWithdrawal(): Result<Unit> =
            runCatching {
                authRemoteDataSource.deleteWithdrawal()
            }

        override suspend fun postReissue(): Result<Unit> =
            runCatching {
